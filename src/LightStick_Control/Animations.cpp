...
void Animation::aRainbow() {
  ...
  fill_rainbow( leds, NUM_LEDS, hueVal );
  hueVal += hueInc;
}
...
void Animation::aGlitter() {
  if( random8() < chanceAct ) {
    ...
    posVal %= NUM_LEDS;
    leds[posVal] = CHSV( hueVal, 255, 255 );
  }
...
void Animation::aConfetti() {
  ...
  leds[posVal] += CHSV( hueVal, 200, 255 );
}
...
void Animation::aBPM() {
  ...
  for( int i = 0; i < NUM_LEDS; i++) { //9948
    leds[i] = ColorFromPalette(palette, hueVal+(i*2), beat-hueVal+(i*10));
  }
...
void Animation::aWhite() {
  // FastLED's built-in solid fill (colorutils.h)
  fill_solid( leds, NUM_LEDS, CHSV(0, 0, hueVal) );
  hueVal += hueInc;
}
